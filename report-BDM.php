      ...
      <ul class="app-menu">
       <li><a class="app-menu__item" href="home-BDM.php"><i class="app-menu__icon fa fa-dashboard"></i><span class="app-menu__label">Home</span></a></li>
        ...
        <li><a class="app-menu__item " href="Accepted-Request-BDM.php"><i class="app-menu__icon fa fa-pie-chart"></i><span class="app-menu__label">Accepted Request</span></a></li>
         ...
         <li><a class="app-menu__item active" href="report-BDM.php"><i class="app-menu__icon fa fa-pie-chart"></i><span class="app-menu__label">Print Report</span></a></li>
      </ul>
    </aside>
...
while($row=mysqli_fetch_array($query)){
  ...
  if (($review1=='acceptedMD' && $review2=='acceptedPM')) {
    $query2="select Email,Name from employee where Status='Manager Directory'";
    $query3="select Email,Name from employee where Status='Project Manager'";
    $run2=mysqli_query($con,$query2);
    $run3=mysqli_query($con,$query3);
    while ($rows=mysqli_fetch_array($run2)) {
      $jibu1=$rows[1];
  ...
  if (($review1=='acceptedBDM' && $review2=='acceptedMD')) {
    $query2="select Email,Name from employee where Status='Business Development Manager'";
    $query3="select Email,Name from employee where Status='Manager Directory'";
    $run2=mysqli_query($con,$query2);
    $run3=mysqli_query($con,$query3);
    while ($rows=mysqli_fetch_array($run2)) {
...
?>
  ...
  <button type="sumit" class="btn btn-block btn-primary btn-lg" onclick="PrintContent('MyDocument')">Print Report</button>
            </div>
          </div>
        </div>
